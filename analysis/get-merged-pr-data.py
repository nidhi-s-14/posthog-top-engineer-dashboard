...
DEFAULT_OUTPUT = Path("analysis/results/merged-pr-summary.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fetch merged pull request info with minimal GitHub API usage.",
    )
    ...
    parser.add_argument(
        ...
        help="Keep PRs merged in the last N days.",
    )
    parser.add_argument(
        "--start-page",
        type=int,
...
def build_headers() -> dict[str, str]:
    headers = {
        ...
        "User-Agent": "posthog-top-engineer-dashboard-merged-pr-fetcher",
...
def build_engineer_summary(pull_requests: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for pull_request in pull_requests:
        ...
        summary[engineer] = summary.get(engineer, 0) + 1

    return [
        {"engineer": engineer, "merged_pr_count": count}
        for engineer, count in sorted(summary.items(), key=lambda item: item[1], reverse=True)
    ]
...
def main() -> int:
    ...
    merged_since = datetime.now(timezone.utc) - timedelta(days=args.days_back)
        ...
        while True:
            ...
            for pull_request in page_pull_requests:
                ...
                if merged_at:
                    ...
                    if merged_dt >= merged_since:
                        simplified_pull_requests.append(simplify_pull_request(pull_request))
                        keep_fetching = True

                if updated_at and parse_github_datetime(updated_at) >= merged_since:
                    keep_fetching = True
    ...
    payload = {
        ...
        "source": {
            ...
            "merged_since": isoformat_z(merged_since),
    ...
    print(f"Wrote {len(simplified_pull_requests)} merged pull requests to {output_path}")
    return 0
